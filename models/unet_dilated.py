...
from models.layers.AtrousSpatialPyramidPool import AtrousSpatialPyramidPool2D
import torch.nn.functional as F

class Unet_Dilation_2D(nn.Module):

    def __init__(self, feature_scale=4, n_classes=1, 
                 dilation=(2, 4, 8), is_deconv=True, is_batchnorm=True):
        super(Unet_Dilation_2D, self).__init__()
        ...
        self.atrousGate = AtrousSpatialPyramidPool2D(filters[3], filters[4], dilation)
    ...
    def forward(self, inputs):
        ...
        # 180602 dilated layer 수정하면서 없애 봤다.
        # center      = self.center(maxpool4)
        pyramidpool = self.atrousGate(maxpool4)

        up4 = self.up_concat4(conv4, pyramidpool)
